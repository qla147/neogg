...
function TestGoods (){
    ...
    GoodsInfoService.checkGoodsInfoAndGoodsDetailParam(goodsInfoOne, goodsDetailOne).then(rs=>{
        assert.equal(rs.success, true , "检测新增商品参数出现错误")
        GoodsInfoService.addGoods({goodsInfo:goodsInfoOne, goodsDetail:goodsDetailOne}).then(rs=>{
            assert.equal(rs.success, true , "新增商品出现错误")
            GoodsInfoService.search({goodsName: goodsInfoOne.goodsName}).then(rs=>{
                assert.equal(rs.success, true , "检索商品列表出现错误")
                assert.equal(!!rs.data , true , "检索商品列表出现逻辑错误，刚加入的商品没有找到" )
                assert.equal(rs.data.count , 1 , "检索商品列表出现逻辑错误，刚加入的商品没有找到")
                assert.equal(rs.data.list.length  , 1 , "检索商品列表出现逻辑错误，刚加入的商品没有找到")
                let goodsInfo = rs.data.list[0]
                GoodsNumRedisModel.getCount(goodsInfo._id.toString()).then(rs=>{
                    assert.equal(rs.success, true , "获取新增商品的库存量，出现错误")
                    assert.equal(rs.data === goodsInfo.goodsCount , true , "新增商品在没有消耗的情况，出现库存不一致")
                    goodsInfoOne.goodsCount = 5
                    GoodsInfoService.updateGoods(goodsInfo._id.toString(), {goodsInfo : goodsInfoOne, goodsDetail: goodsDetailOne}).then(rs=>{
                        assert.equal(rs.success, true , "变更商品数量出现错误")
                        GoodsLockRedisModel.status(goodsInfo._id.toString()).then(rs=>{
                            assert.equal(rs.success, true , "变更商品完成，检测商品锁出现错误")
                            assert.equal(!rs.data, true , "变更商品完成，锁应该被释放，但是商品锁还在存在"  )
                            GoodsNumRedisModel.getCount(goodsInfo._id.toString()).then(rs=>{
                                assert.equal(rs.success, true , "获取变更商品之后的库存量，出现错误")
                                assert.equal(rs.data === goodsInfo.goodsCount , true , "变更商品之后在没有消耗的情况，出现库存不一致")
                            })
